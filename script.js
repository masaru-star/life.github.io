        ...
        const TILE_SIZE = 30;
        const REAL_SEC_PER_GAME_MIN = 1 / 3; 
        const MS_PER_GAME_MINUTE = 1000 / (3 / REAL_SEC_PER_GAME_MIN); 
        const FURNITURE_TYPES = {
            laptop: { name: "ノートパソコン", width: 1, height: 1, price: 1000, power: 5, use: (p) => { p.stats.fun = Math.min(100, p.stats.fun + 7); p.showSpeech("楽しい！"); }, duration: 60 },
            microwave: { name: "電子レンジ", width: 1, height: 1, price: 400, power: 2, use: (p) => { if (gameState.money >= 75) { gameState.money -= 8; p.stats.hunger += 80; p.showSpeech("おいしい！(8G)"); } else { p.showSpeech("お金がない..."); } } },
            bed: { name: "ベッド", width: 2, height: 1, price: 200, power: 0, water: 0, use: (p) => { p.stats.energy = 100; p.showSpeech("Zzz..."); }, duration: 7 * 60 },
            bath: { name: "風呂", width: 2, height: 1, price: 200, water: 2, use: (p) => { p.stats.hygiene = 100; p.showSpeech("さっぱり"); }, duration: 30 },
            shower: { name: "シャワー", width: 1, height: 1, price: 200, water: 1, use: (p) => { p.stats.hygiene = 100; p.showSpeech("スッキリ"); }, duration: 15 },
            grill: { name: "グリル", width: 1, height: 1, price: 1200, power: 5, water: 0, use: (p) => { /* グリル使用ロジックはPerson.handleActionで処理 */ } },
        };
         ...
         const TRAITS = {
                gentle: { name: "優しい", good: ["gentle", "shy"], bad: ["mischievous"] },
                mischievous: { name: "いたずら好き", good: ["mischievous"], bad: ["shy"] },
                shy: { name: "人見知り", good: ["gentle"], bad: ["serious", "otaku"] },
                serious: { name: "真面目", good: ["serious", "otaku"], bad: ["gentle"] },
                otaku: { name: "ヲタク", good: ["otaku", "gentle"], bad: ["shy"] },
         };
        const JOBS = {
            none: { name: "無職", start: 0, end: 0, pay: 0 },
            part_time_day: { name: "アルバイト(昼間)", start: 9 * 60, end: 11 * 60, pay: 80 },
            part_time_night: { name: "アルバイト(夜間)", start: 20 * 60, end: 22 * 60, pay: 100 },
            office_worker: { name: "会社員", start: 10 * 60, end: 16 * 60, pay: 130 },
            entertainer: { name: "エンターテイナー", start: 13*60, end: 17*60, pay: 80},
        };
        let gameState = {
            time: new Date(2025, 0, 1, 6, 0, 0), // ゲーム内時間 (6時スタート)
            money: 10000,
            dailyWater: 0,
            dailyPower: 0,
            isRunning: false,
            lastUpdateTimestamp: Date.now(), // ゲームループの最終更新時刻
            accumulatedTimeMs: 0, // 蓄積されたリアルタイムミリ秒
            isDesignMode: false,
            foodInventory: { // 料理保存箱
            servings: 0,  // 食数
            quality: 0,   // 品質 (1食あたりの満腹度回復量)
            }
        };

        let people = []; // {id, data: Person}
        let furniture = []; // {id, data: Furniture}
        let roomGrid = Array(ROOM_HEIGHT).fill(null).map(() => Array(ROOM_WIDTH).fill(null)); // 家具の配置情報 (null or furniture.id)
        let selectedPerson = null;
        let gameLoopRAF = null; // requestAnimationFrame のID

        let nextPersonId = 0;
        let nextFurnitureId = 0;

        let selectedFurnitureForMove = null; // 移動選択中の家具
        ...
        class Person {
            constructor(id, firstName, lastName, gender, orientation, traitKey) {
                ...
                this.traitKey = traitKey || this.getRandomTraitKey(); // 特質
                this.trait = TRAITS[this.traitKey];
                this.birthDate = new Date(gameState.time.getTime()); // 作成日を誕生日として記録
                const charSize = 20; // キャラクターのサイズ (CSSより)
                this.x = Math.random() * (ROOM_WIDTH * TILE_SIZE - charSize); // 修正
                this.y = Math.random() * (ROOM_HEIGHT * TILE_SIZE - charSize); // 修正

                this.stats = {
                    hunger: 80, // 食欲 (100満腹 -> 0空腹)
                    energy: 80, // 睡眠欲 (100元気 -> 0眠い)
                    hygiene: 80, // 清潔 (100清潔 -> 0不潔)
                    fun: 80,     // 楽しさ (100楽しい -> 0退屈)
                };
                
                this.job = JOBS.none;
                this.isWorking = false;
                this.relationships = {}; // 他のperson.idとの関係値 (-100 ~ 100)
                this.skills = {
                    talk: { level: 0, exp: 0 },
                    game: { level: 0, exp: 0 },
                    job: { level: 0, exp: 0 },
                ...
                this.currentAction = null; // 例: { type: 'sleep', target: furniture, duration: 420 }
                this.lastExpCheckTime = 0;
                this.targetPosition = null; // 移動目標の {x, y}
                this.moveSpeed = 1.2; // 1ゲーム分あたりの移動ピクセル数
                this.initDOM();
            }
            get nameKey() {
            ...
            toSaveData() {
                return {
                    ...
                    y: this.y,
                    stats: this.stats,
                    jobKey: Object.keys(JOBS).find(key => JOBS[key] === this.job), // jobオブジェクトでなくキーを保存
            ...
            initDOM() {
                ...
                this.element.innerText = this.firstName.charAt(0); // イニシャル
                this.element.onclick = (e) => {
                    e.stopPropagation(); // 親(部屋)へのクリック伝播を防ぐ
                    selectPerson(this);
                };
            ...
            update(gameMinutesPassed) {
                ...
                if (this.currentAction) {
                    ...
                    return; // アクション中は他のAIは動かない
                }
                
                // 3. 移動処理
                if (this.targetPosition) {
                    this.move(gameMinutesPassed);
                    return; // 移動中は他のAIは動かない
                }
            ...
            updateNeeds(gameMinutesPassed) {
                this.stats.hunger = Math.max(0, this.stats.hunger - (gameMinutesPassed / 60) * 2); // 2時間で約4減少
            ...
            // AI: 次の行動を決める
            findNewAction() {
                // 優先順位1: 緊急の欲求 (食事 > 睡眠 > 清潔)
                if (this.stats.hunger < 30) {
        // 1. 保存箱の料理があるかチェック（新しいメソッドを呼び出す）
        if (this.tryUseFoodInventory()) return; // <--- 新規
        
        // 2. グリルがあるかチェック（調理）
        if (this.tryFindAndUseFurniture('grill')) return; // <--- 新規

        // 3. 電子レンジがあるかチェック（優先度を下げる）
        if (this.tryFindAndUseFurniture('microwave')) return; 
                ...
                // 会話相手がいないならPC (ノートパソコン)
                if (this.tryFindAndUseFurniture('laptop')) return;

                // 優先順位3: ランダムな移動 (要望: 常に動き回る)
                // 上記の行動を何も見つけられなかった場合、必ずランダムに移動する (確率を撤廃)
                const charSize = 20;
                this.setTargetPosition(
                    Math.random() * (ROOM_WIDTH * TILE_SIZE - charSize), // 修正
                    Math.random() * (ROOM_HEIGHT * TILE_SIZE - charSize) // 修正
                );
            }
            ...
            tryFindAndUseFurniture(typeKey) {
                ...
                if (targetFurniture) {
                    targetFurniture.isUsed = true; // すぐに使用中にして他の人が使わないように
                    ...
                    this.setTargetPosition(usePos.x, usePos.y, () => {
                        ...
                        if (typeKey === 'grill') {
                            ...
                            // レベル0: 60分, レベル1ごと: 5分短縮
                            duration = Math.max(5, 60 - cookLevel * 5); // 最低5分
                            this.currentAction.duration = duration; // ★継続時間を上書き
                            this.showSpeech(`調理開始！(${duration}分)`);
                        }
                        this.lastExpCheckTime = 0;
                        // 使用開始時に関数を1回実行
            ...
            startConversation(other) {
                ...
                // 会話スキル: 高いほど仲の良さ値が減りにくくなる (負の補正を緩和)
                if (baseChange < 0) {
                    baseChange *= (1 - (myTalkLevel * 0.05)); // 最大50%減少緩和 (Lv10で)
                }
                
                // ゲームスキル: 相手が「ヲタク」なら仲の良さ値が増えやすくなる
                if (other.traitKey === 'otaku') {
                    baseChange *= (1 + (this.skills.game.level * 0.05)); // 最大50%増加 (Lv10で)
                }
                
                // 職業スキル: 相手が「真面目」なら仲の良さ値が増えやすくなる
                if (other.traitKey === 'serious') {
                    baseChange *= (1 + (this.skills.job.level * 0.05)); // 最大50%増加 (Lv10で)
                }
                ...
                // 会話アクションを設定 (両者) - change値をアクションに保存
                const duration = 15; // 15分会話
                ...
                this.gainSkillExp('talk', Math.floor(duration / 3)); // 3分会話で1exp
                other.gainSkillExp('talk', Math.floor(duration / 3));
                // 会話開始の吹き出し
                this.showSpeech("こんにちは！");
                other.showSpeech("どうも！");

                // 終了メッセージタイマーをセット
                setTimeout(() => {
                    this.showRelationshipChange(change, other);
                }, duration * MS_PER_GAME_MINUTE * 2); // duration * MS_PER_GAME_MINUTE = ゲーム内時間、2倍はバッファ
            }
            ...
            // 新規メソッド: 関係値の変動を吹き出し表示
            showRelationshipChange(change, otherPerson) {
                let message;
                let otherMessage;
                
                if (change > 0) {
                    message = `${otherPerson.firstName}と仲良くなった！ (+${change})`;
                    otherMessage = `${this.firstName}と仲良くなった！ (+${change})`;
                } else {
                    message = `${otherPerson.firstName}と気まずい... (${change})`;
                    otherMessage = `${this.firstName}と気まずい... (${change})`;
                }
            ...
            handleAction(gameMinutesPassed) {
                ...
                if (intervalsPassed > 0) {
                    ...
                    // 修正: ノートパソコン使用時の経験値獲得ロジック
                    if (this.currentAction.target.type === 'laptop') {
                        const expBlockSize = 5; // 5分で1Exp
                        
                        // 経過時間を計算 (duration:初期時間, timer:残り時間)
                        // ★タイマーは増加していく方式 (timer: 0 -> duration)
                        const elapsedTime = this.currentAction.timer; 
                        
                        // 初回実行時の初期化 (lastExpGainTimeが未定義の場合、0に設定)
                        ...
                        if (expToGain > 0) {
                            this.gainSkillExp('game', expToGain); // 経験値を獲得
                            // 最後に経験値を獲得した「経過時間」を更新
                ...
                if (this.currentAction.timer >= this.currentAction.duration) {
                    ...
                    if (target && target.isUsed) { // 家具の場合
                        target.isUsed = false; // 家具を解放
                    }
                    
                    if (this.currentAction.type === 'talk') {
                        // 相手がまだゲーム内に存在するか（IDでpeople配列を検索）
                        ...
                        if (otherExists) { 
                            ...
                            if (relationshipValue > -40) {
                                this.stats.fun = Math.min(100, this.stats.fun + 10); // 10回復
                            }
                        }
                    }
                    // --- ここまで ---

                    if (target && target.type === 'grill') { // 家具かつグリル
                        // 1. 資金消費
                        gameState.money -= 10;
                        ...
                        // 3. 料理の品質と食数を設定
                        gameState.foodInventory.servings += 4; // 4食分
                        // 1食あたり満腹度: 10*完成度+30 (100以上なら100)
                        const hungerRecovery = Math.min(100, 10 * quality + 30); 
                        gameState.foodInventory.quality = hungerRecovery; 
                        
                        // 4. 調理スキル経験値獲得
                        this.gainSkillExp('cook', 12); // 1回で12exp
                        
                        this.showSpeech(`調理完了！品質 ${quality} の料理が4食できた！`);
                    }
            ...
            checkWorkStart() {
                ...
                if (gameTimeMinutes >= this.job.start && gameTimeMinutes < this.job.end) {
                    this.isWorking = true;
                    this.element.style.display = 'none'; // 画面から消す
                    // --- 修正3: 職業経験値 (削除) ---
                    // this.gainSkillExp('job', 5); // (checkWorkEndに移動)
                    // --- ここまで ---
                }
            }
            // 保存箱の料理を食べる試行
            tryUseFoodInventory() {
                if (gameState.foodInventory.servings > 0) {
                    ...
                    this.showSpeech(`保存食を食べた！(残り${gameState.foodInventory.servings}食)`);
                    return true;
                }
                return false;
            }
            ...
            setJob(jobKey) {
                ...
                this.showSpeech(`${this.job.name} になった！`);
            ...
            checkWorkEnd() {
                ...
                if (gameTimeMinutes >= this.job.end || gameTimeMinutes < this.job.start) {
                    this.isWorking = false;
                    this.element.style.display = 'flex'; // 再表示
                    const hoursWorked = (this.job.end - this.job.start) / 60;
                    
                    // --- 修正6: 時給計算 (職業レベル×20＋固定値) ---
                    ...
                    // --- 修正3: 職業経験値 (ここに追加) ---
                    this.gainSkillExp('job', 5); // 1回出勤で5exp
                    // --- ここまで ---
                    
                    this.showSpeech(`仕事終わり！ ${Math.floor(earnings)}G 稼いだ！`);
                    updateUI();
                }
            }
            
            // 相手とのインタラクションポイント (相手の隣)
            getInteractionPoint(otherPerson) {
                // キャラクターの左上座標にタイルサイズを足して隣のタイルの中心を狙う
            ...
            gainSkillExp(skillKey, exp) {
                ...
                // レベルアップのしきい値（簡易的な設定: 1レベルあたり100exp）
                const expNeededForNextLevel = (currentLevel + 1) * 100; 

                // レベル10が最大
                if (currentLevel < 10 && skill.exp >= expNeededForNextLevel) {
                    skill.level++;
                    this.showSpeech(`${skillKey}スキルが${skill.level}にレベルアップ！`);
                    // 次のレベルの経験値が足りるか再チェック（連続レベルアップ用）
                    if (skill.level < 10) {
                        this.gainSkillExp(skillKey, 0); // 再帰呼び出し
                    }
                }
                
                // レベル10以上の場合、経験値は蓄積するがレベルは上がらない
            }
            // 描画
            draw() {
        ...
        class Furniture {
            ...
            toSaveData() {
                return {
                    ...
                    gridX: this.gridX,
                    gridY: this.gridY,
                    // isUsedは保存しない（ロード時に解放状態になるため）
                };
            }
            
            // 保存データから復元
            static fromSaveData(data) {
                // DOM描画もコンストラクタ内で実行される
            ...
            // グリッドに家具を描画（色塗り）
            draw() {
                this.removeHighlight(); // 古いハイライトを削除
                const color = FURNITURE_COLORS[this.type] || '#ccc'; // 色を取得
            ...
            clearGridInfo() {
                for (let y = 0; y < this.height; y++) {
                    for (let x = 0; x < this.width; x++) {
                        if (roomGrid[this.gridY + y] && roomGrid[this.gridY + y][this.gridX + x] !== undefined) { // 範囲チェック修正
        ...
        function gameLoop() {
            if (!gameState.isRunning) return; // 念のため
        ...
        function updatePeople(gameMinutesPassed) {
            // people 配列のコピーに対してループ (削除中にイテレータが壊れるのを防ぐ)
            // ※削除処理(deleteSelectedPerson)が即時実行されるため、安全策
        ...
        function updateUI() {
            ...
            timeDisplayEl.innerText = `時刻: ${days}日目 ${hours}:${minutes}`;

            // 所持金
            moneyDisplayEl.innerText = `所持金: ${Math.floor(gameState.money)} G`;

            // リソース
            resourceDisplayEl.innerText = `水: ${gameState.dailyWater} / 電: ${gameState.dailyPower} | 料理: ${gameState.foodInventory.servings}食 (満腹度:${gameState.foodInventory.quality})`;            
            // 選択中の人
            updateSelectedPersonUI();
            ...
            if (gameState.isDesignMode) {
                designModeStatusEl.innerText = selectedFurnitureForMove ? "模様替えモード: 家具移動中" : "模様替えモード";
                designModeStatusEl.style.color = '#28a745';
            } else if (selectedFurnitureForMove) {
                 // 模様替えOFFでも移動中はメッセージを表示
                 designModeStatusEl.innerText = "移動先のタイルをクリックしてください";
        ...
        function updateSelectedPersonUI() {
            if (selectedPerson) {
                // 仲の良さリストを生成
                let relationshipHTML = '<h5>仲の良さ:</h5>';
                ...
                for (const other of people) {
                    ...
                    // -100 ~ 100 の値を 0 ~ 100 の表示幅に変換
                    const barWidth = Math.abs(relationshipValue);
                    // 負の値の場合は左寄せ（左側から伸びる）、正の値の場合は右寄せ（右側から伸びる）
                    const marginLeft = relationshipValue < 0 ? 50 - barWidth : 50; 
                    const statColor = relationshipValue >= 0 ? '#17a2b8' : '#6c757d'; // 青系: 仲良し, 灰色系: 気まずい
                    
                    // relationship クラスを付与して、中央線を表示するカスタムCSSの余地を残す
                    relationshipHTML += `
                ...
                if (!hasRelationships) {
                    relationshipHTML = '<div>まだ他の人はいません。</div>';
                }

                let statsHTML = `
                    <h4>${selectedPerson.lastName} ${selectedPerson.firstName}</h4> 
                    <div>職業: ${selectedPerson.job.name}</div>
                    <div>特質: ${selectedPerson.trait.name}</div>
                    <div class="stat-label">食欲 (${Math.floor(selectedPerson.stats.hunger)})</div>
                    <div class="stat-bar"><div style="width: ${selectedPerson.stats.hunger}%; background: #f44336;"></div></div>
                    <div class="stat-label">睡眠 (${Math.floor(selectedPerson.stats.energy)})</div>
                    <div class="stat-bar"><div style="width: ${selectedPerson.stats.energy}%; background: #2196F3;"></div></div>
                    <div class="stat-label">清潔 (${Math.floor(selectedPerson.stats.hygiene)})</div>
                    <div class="stat-bar"><div style="width: ${selectedPerson.stats.hygiene}%; background: #00BCD4;"></div></div>
                    <div class="stat-label">楽しさ (${Math.floor(selectedPerson.stats.fun)})</div>
                    <div class="stat-bar"><div style="width: ${selectedPerson.stats.fun}%; background: #FFEB3B;"></div></div>
                    <button id="assign-job-modal-btn">仕事</button>
                    <button id="delete-person-btn" style="background-color: #dc3545; margin-left: 10px;">削除</button> <hr style="margin: 10px 0;">
                    <hr style="margin: 10px 0;">
                    <h5>スキルレベル:</h5>
                    <div class="stat-label">会話: Lv ${selectedPerson.skills.talk.level} (Exp: ${selectedPerson.skills.talk.exp})</div>
                    <div class="stat-label">ゲーム: Lv ${selectedPerson.skills.game.level} (Exp: ${selectedPerson.skills.game.exp})</div>
                    <div class="stat-label">職業: Lv ${selectedPerson.skills.job.level} (Exp: ${selectedPerson.skills.job.exp})</div>
                    <div class="stat-label">調理: Lv ${selectedPerson.skills.cook.level} (Exp: ${selectedPerson.skills.cook.exp})</div>
                    <hr style="margin: 10px 0;">
                    ${relationshipHTML}
                `;
                ...
                document.getElementById('delete-person-btn').onclick = () => {
                    if (confirm(`${selectedPerson.lastName} ${selectedPerson.firstName}を削除しますか？`)) {
                        deleteSelectedPerson();
                    }
                };
            } else {
                // 選択されていない場合はシンプルなメッセージを表示
                personDetailsEl.innerHTML = '<h4>選択中の人</h4>（部屋のキャラクターをクリックして選択）'; 
            }
        }
...
function deleteSelectedPerson() {
    ...
    // 3. 他のキャラクターの関係性リストから削除された人の情報を消す（条件付き）
    for (const person of people) {
        // deletedNameKeyはここで正しく参照されます
        const relationshipValue = person.relationships[deletedNameKey] || 0; 
        
        // 仲の良さ値が -5 ～ 5 の範囲外なら残す
        if (relationshipValue > 5 || relationshipValue < -5) {
            // 削除しない (将来の同姓同名のための復元用)
        } else {
            delete person.relationships[deletedNameKey];
        }

        // --- 修正2: 削除された人が会話や移動のターゲットになっていないかチェックし、リセット ---
        // この処理は for ループの内側に入れることで、person が参照可能になります。
        
        // currentActionのターゲットが削除された人自身の場合
        if (person.currentAction && person.currentAction.target && person.currentAction.target.id === deletedId) {
            // targetがPersonの場合（会話など）
            person.currentAction = null;
        } 
        // targetPositionのtargetIdが削除された人のIDの場合 (※targetIdというプロパティは現状ないが、将来的に)
    ...
    updateSelectedPersonUI();
        ...
        function confirmAddPerson() {
            const firstName = document.getElementById('p-firstname').value || "名無し";
            const lastName = document.getElementById('p-lastname').value || "姓";
            ...
            for (let p of people) {
                ...
                if (foundOldKey) {
                    ...
                    restoredRelationships[p.nameKey] = relationshipValue; // 新しい人から見た関係性
                    delete p.relationships[foundOldKey]; // 古いキーを削除
                } else {
                    // 引き継ぐ関係性がない場合は0で初期化
            ...
            newPerson.relationships = restoredRelationships; // 復元（または初期化）した関係性を設定
            // --- ここまで ---
        ...
        function toggleDesignMode() {
            ...
            if (gameState.isDesignMode) {
                ...
                hideDesignModal();
                cancelFurnitureMove(); // 模様替えモード終了時に移動もキャンセル
            }
            updateUI(); // ステータス表示更新
        }
        ...
        function updateFurnitureInfo() {
            ...
            if (!key) return;
            const f = FURNITURE_TYPES[key];
            furnitureInfo.innerText = `価格: ${f.price} G | サイズ: ${f.width}x${f.height}`;
        }

        // 指定された座標に家具を設置可能かチェック
        ...
        function buyFurniture() {
            ...
            if (!canPlaceFurniture(definition, x, y)) {
                alert("指定された場所には家具を設置できません（範囲外または他の家具と衝突）。");
                return;
            }
        ...
        function selectFurnitureToMove(furnitureId) {
            cancelFurnitureMove(); // 既に選択中の家具があれば解除
        ...
        function moveSelectedFurniture(targetX, targetY) {
            ...
            if (!canPlaceFurniture(selectedFurnitureForMove.definition, targetX, targetY, selectedFurnitureForMove.id)) {
                alert("指定された場所には移動できません（範囲外または他の家具と衝突）。");
                return;
            }
            ...
            selectedFurnitureForMove.draw(); // DOM上の表示を更新
            cancelFurnitureMove(); // 移動完了で選択解除
        }
        ...
        function openJobModal(person) {
            ...
            document.getElementById('job-modal-title').innerText = `${person.lastName} ${person.firstName}の仕事`;
        ...
        function init() {
            ...
            for (let y = 0; y < ROOM_HEIGHT; y++) {
                for (let x = 0; x < ROOM_WIDTH; x++) {
                    ...
                    tile.id = `tile-${x}-${y}`;
                    tile.dataset.gridX = x; // 座標をデータ属性として追加
            ...
            designModeBtn.addEventListener('click', toggleDesignMode); // 模様替えモード切り替え
            ...
            document.getElementById('exit-design-mode-btn').addEventListener('click', toggleDesignMode); // 模様替えモード終了
            ...
            // --- 修正7: グリッドタイルへのイベントリスナー (家具移動用) ---
            roomGridEl.addEventListener('dblclick', (e) => {
                // if (!gameState.isDesignMode) return; // <-- ガードを削除
                ...
                if (targetTile && targetTile.dataset.furnitureId) {
                    ...
                    if (gameState.isDesignMode) {
                        ...
                        // 模様替えモードOFF: 確認メッセージを表示
                        const targetFurniture = furniture.find(f => f.id === furnitureId);
                        // 移動中でなく、家具が使用中でもないこと
                        if (targetFurniture && !selectedFurnitureForMove && !targetFurniture.isUsed) { 
                            if (confirm(`${targetFurniture.definition.name} を移動しますか？ (使用中の家具は移動できません)`)) {
                                selectFurnitureToMove(furnitureId);
                                updateUI(); // ステータス表示を更新
                            }
                        } else if (targetFurniture && targetFurniture.isUsed) {
                            alert("使用中の家具は移動できません。");
                        }
                    }

                } else if (gameState.isDesignMode) {
                    // 模様替えモード中で家具のない場所をダブルクリックしたら移動選択解除
                    cancelFurnitureMove();
                }
            });

            roomGridEl.addEventListener('click', (e) => {
                // if (!gameState.isDesignMode || !selectedFurnitureForMove) return; // <-- 変更
                
                if (!selectedFurnitureForMove) return; // 移動対象がなければ何もしない
                ...
                if (targetTile) {
                    ...
                    // 模様替えモードOFFで移動した場合、UIを即時更新してメッセージを消す
                    if (!gameState.isDesignMode) {
                        updateUI();
                    }
                }
        ...
        function saveGame() {
            ...
            const saveData = {
                // 1. 基本状態
                time: gameState.time.getTime(), // Dateオブジェクトをタイムスタンプとして保存
            ...
            // JSON文字列に変換してテキストエリアに表示
            const jsonString = JSON.stringify(saveData, null, 2);
            saveDataArea.value = jsonString;
            alert("ゲーム状態を保存しました。テキストエリアのJSONをコピーしてください。");
        }
        ...
        function loadGame() {
            try {
                ...
                // 3. 家具データを復元し、グリッド情報も再構築
                furniture = saveData.furniture.map(fData => Furniture.fromSaveData(fData));
                rebuildRoomGrid();

                // 4. その他状態のリセット/更新
                selectPerson(null);
                gameState.isRunning = false; // ロード時は時間を停止
                timeToggleBtn.innerText = "時間を進める";
                timeToggleBtn.classList.remove('ticking');
                if (gameLoopRAF) cancelAnimationFrame(gameLoopRAF); // ゲームループを停止
